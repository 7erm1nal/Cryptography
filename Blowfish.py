...
parser=argparse.ArgumentParser(prog="Blowfish",
description='A CLI-realization of cryptograpgic algirithm "Blowfish"', 
epilog='Examples: python Blowfish.py -a encrypt "rats rats rats" keysword / python Blowfish.py -a decrypt "ca c3 76 e4 03 12 02 dd d8 4f 8b 58 9b 63 f8 03" keysword', 
add_help=True)

parser.add_argument("-a", "--action", choices=("encrypt", "decrypt"), default="encrypt", dest="act", help="Type of action. Choose encrypt or decrypt")
parser.add_argument("text", type=str, help="A cleantext or siphertext")
parser.add_argument("key", type=str, help="A key to encrypt cleantext or decrypt siphertext\nLength 4-56 symbols")
...
def text_to_num(text=str):
    """
    Преобразование строки текста из  символов в двоичное, а затем в десятичное представление
    """
    return int.from_bytes(text.encode(encoding="ascii", errors="replace"), byteorder='big')

def hex_to_num(text=str):
    """
    Преобразование строки из  байтов в двоичное, а затем в десятичное представление
    """
    text=bytes.fromhex(text)
    return int.from_bytes(text, byteorder='big')

def num_to_text(num:int): 
    """
    Преобразование десятичного числа в ascii символы
    """   
    text=binascii.unhexlify('%x' % num)
    text=str(text)
    return text[2:-1]
    
def sipher_output(sipher:int):
    """
    Преобразование десятичного числа в цепочку байтов
    """    
    sipher=binascii.unhexlify('%x' % sipher).hex()
    sipher=[' '.join(sipher[i:i + 2] for i in range(0, len(sipher), 2))]
    return sipher[0]

def P_keyarray(key=str):
    """
    Генерация массива чисел 18-ти ключей Blowfish
    """
    key=[key[x:x+4] for x in range(0, len(key), 4)] #Нарезка текста ключа на блоки по 4 символа (32 бита)
    if len(key[-1]) < 4:                            #Обрезка неполной "четверки" 
        key=key[:-1]
    array=[]
    for i in range(18):                             #Генерация 18-ти значений
        array.append(hex_to_num(P[i]) ^ text_to_num(key[i % len(key)]))#XOR байтов из таблицы P и четверки символов ключа
    return array
...
def F(part:int):
    ...
    Функция Фейстеля
    """
    #Разделение на 4 блока по 8 бит
    s1 = (part & 0b11111111000000000000000000000000)>>24
    s2 = (part & 0b00000000111111110000000000000000)>>16
    s3 = (part & 0b00000000000000001111111100000000)>>8
    s4 =  part & 0b00000000000000000000000011111111
    #Подстановки по таблице S
    s1=S[0][s1]
    s2=S[1][s2]
    s3=S[2][s3]
    s4=S[3][s4]
    ...
    s4=hex_to_num(s4)
    #Основная формула
    result=((((s1+s2) % 2**32) ^ s3) + s4) % 2**32
    return result

def Encrypt(text:str, key:str):
    if 4<=len(key)<=56:
        keyarray=P_keyarray(key)    #Генерация массива ключей
    else:
        print("The key is not in range (4 to 56)")
        return 0
    
    text=[text[x:x+8] for x in range(0, len(text), 8)]  #Нарезка на блоки по 8 символов (64 бита)
    if len(text[-1])<8:
        text[-1]=text[-1]+(' '*(8-len(text[-1])))       #Добавление пробелов для кратности с 64
    text=[text_to_num(x) for x in text]                 #Перевод в числа
    
    mask_left =0b1111111111111111111111111111111100000000000000000000000000000000
    mask_right=0b0000000000000000000000000000000011111111111111111111111111111111

    sipher=[]
    for i in range(len(text)):
        L=(text[i] & mask_left)>>32       #Разделение на левую и правую части по маскам
        R=text[i] & mask_right
        for j in range(16):               #16 основных раундов шифрования
            L=L ^ keyarray[j]
            L, R=F(L) ^ R, L
        L, R=R, L
        L=L^keyarray[17]                  #Последние 2 XORа левой и правой частей
        R=R^keyarray[16]
        sipher.append(L<<32 | R)
    
    for i in range(len(sipher)):          #Нормальный вывод шифротекста цепочкой байтов
        sipher[i]=sipher_output(sipher[i])
    sipher = ' '.join(sipher)
    return sipher

def Decrypt(sipher:str, key:str):
    if 4<=len(key)<=56:
        keyarray=P_keyarray(key)    #Обычная генерация массива ключей
        keyarray=keyarray[::-1]     #Переворачиваем массив ключей чтобы пустить в обратном порядке
    else:
        print("The key is not in range (4 to 56)")
        return 0
    
    sipher = sipher.replace(' ', '')        #Удаление пробелов
    sipher = [sipher[x:x+16] for x in range(0, len(sipher), 16)]    #нарезка на блоки по 8 байт
    sipher = [hex_to_num(x) for x in sipher]       #Перевод в десятичные числа
    ...
    for i in range(len(sipher)):
        L=(sipher[i] & mask_left)>>32       #Разделение на левую и правую части по маскам
        R=sipher[i] & mask_right
        for j in range(16):                 #Основные 16 раундов 
            L=L ^ keyarray[j]
            L, R=F(L) ^ R, L
        L, R=R, L
        L=L^keyarray[17]                    #Последний XOR
        R=R^keyarray[16]
        text.append(L<<32 | R)

    text=[num_to_text(x) for x in text]     #Перевод в символьный формат
    text=''.join(text)
    return(text)
...
if __name__=="__main__":
    ...
    elif args.act=="decrypt":
        text=args.text
        text=text.split()
        if (len(text)%8 == 0) and (all(len(byte)==2 for byte in text)):
            print(Decrypt(args.text, args.key))
        else:
            print('Error. Input: String of bytes\nFormat: "FF FF FF FF FF FF ..."')
    else:
        print("Error. What action do you need?\nType encrypt or decrypt with flag -a.")